...
static Sym *external_sym(int v, CType *type, int r, AttributeDef *ad)
{
    ...
    if (!s) {
        ...
        /* discovery mode hook for TccLua */
        if (tcc_state->discovery_mode && tcc_state->on_discovery_sym) {
            if (v > 0 && (v & ~SYM_FIELD)) {
                const char *name = get_tok_str(v, NULL);
                tcc_state->on_discovery_sym(tcc_state, name, s);
            }
        }
    } else {
...
static void struct_decl(CType *type, int u)
{
    ...
    int struct_tag_v = 0; /* saved struct tag for discovery callback */
    Sym *s, *ss, **ps;
    AttributeDef ad, ad1;
    CType type1, btype;
    ...
    if (tok >= TOK_IDENT) /* struct/enum tag */
        v = tok, next();
    struct_tag_v = v; /* save for discovery callback */
    ...
    if (tok == '{') {
	    ...
	    CType t;
			...
			get_tok_str(v, NULL));
                ...
                ss->enum_val = ll;
                /* discovery mode hook for TccLua - enum constant */
                if (tcc_state->discovery_mode && tcc_state->on_discovery_enum) {
                    const char *name = get_tok_str(v, NULL);
                    tcc_state->on_discovery_enum(tcc_state, name, ll);
                }
                *ps = ss, ps = &ss->next;
                if (ll < nl)
                    nl = ll;
        ...
        /* discovery mode hook for TccLua - struct/union/enum */
        if (tcc_state->discovery_mode && tcc_state->on_discovery_sym) {
            if (struct_tag_v > 0 && (struct_tag_v & ~SYM_FIELD)) {
                const char *name = get_tok_str(struct_tag_v & ~SYM_STRUCT, NULL);
                tcc_state->on_discovery_sym(tcc_state, name, s);
            }
        }
    }
