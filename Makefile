 ...
 ifeq ($(CONFIG_static),no)
  ...
  ifneq ($(CONFIG_rpath),no)
    ifndef CONFIG_OSX
      ...
      # macOS doesn't support env-vars libdir out of the box - which we need when
      # libtcc.dylib is used (configure --disable-static), so
...
TCC_X = i386 x86_64 i386-win32 x86_64-win32 x86_64-osx arm arm64 arm-wince
TCC_X += riscv64 arm64-osx
# TCC_X += arm-fpa arm-fpa-ld arm-vfp arm-eabi

# cross libtcc1.a targets to build
LIBTCC1_X = $(filter-out,$(TCC_X))
...
# run all tests from deps_test/tccfork submodule
TESTDIR = deps_test/tccfork/tests
test: $(PROGS) $(TCCLIBS)
	@ec=0; $(MAKE) -C $(TESTDIR) TOP=$(CURDIR) TOPSRC=$(CURDIR)/deps_test/tccfork all || ec=$$?; \
	git -C deps_test/tccfork clean -fdx tests/ >/dev/null 2>&1; \
	exit $$ec

# run test(s) from tests2 subdir (see make help)
tests2.%:
	@ec=0; $(MAKE) -C $(TESTDIR)/tests2 TOP=$(CURDIR) TOPSRC=$(CURDIR)/deps_test/tccfork $@ || ec=$$?; \
	git -C deps_test/tccfork clean -fdx tests/ >/dev/null 2>&1; \
	exit $$ec

# run test(s) from pp subdir (see make help)
testspp.%:
	@ec=0; $(MAKE) -C $(TESTDIR)/pp TOP=$(CURDIR) TOPSRC=$(CURDIR)/deps_test/tccfork $@ || ec=$$?; \
	git -C deps_test/tccfork clean -fdx tests/ >/dev/null 2>&1; \
	exit $$ec

# test the installed tcc instead
test-install: $(TCCDEFS_H)
	@ec=0; $(MAKE) -C $(TESTDIR) TOP=$(CURDIR) TOPSRC=$(CURDIR)/deps_test/tccfork TESTINSTALL=yes || ec=$$?; \
	git -C deps_test/tccfork clean -fdx tests/ >/dev/null 2>&1; \
	exit $$ec
...
help:
	...
	@echo "make test"
	@echo "   run all tests from deps_test/tccfork submodule"
